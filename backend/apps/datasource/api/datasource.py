...
async def check_by_id(session: SessionDep, trans: Trans, ds_id: int):
    try:
        ...
        return await asyncio.to_thread(inner)
        #return {"code": 0, "data": True, "msg": 'xxxxx'}
    except Exception:
        return {"code": 0, "data": True, "msg": None}
...
async def exec_sql(session: SessionDep, id: int, obj: TestObj):
    def inner():
        ...
        try:
            ...
            SQLBotLogUtil.error("Failed to execute SQL in test endpoint", exc_info=True)
...
def insert_pg(df, tableName, engine):
    ...
    try:
        ...
        SQLBotLogUtil.error("Failed to import CSV", exc_info=True)
        raise HTTPException(status_code=400, detail="Import failed, please retry later")
    finally:
        cursor.close()
        conn.close()
