...
def get_uri(ds: CoreDatasource) -> str:
    if ds.type == "excel":
        ...
        if not ds.configuration:
            raise ValueError(f"Datasource {ds.id} has empty configuration")
        try:
            decrypted_config = aes_decrypt(ds.configuration)
            conf = DatasourceConf(**json.loads(decrypted_config))
        except (ValueError, json.JSONDecodeError) as e:
            SQLBotLogUtil.error(f"Failed to decrypt/parse configuration for datasource {ds.id}: {e}")
            raise ValueError(f"Invalid datasource configuration: {str(e)}") from e
