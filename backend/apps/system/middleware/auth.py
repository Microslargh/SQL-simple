...
class TokenMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request, call_next):
        ...
        # 如果OAuth2已启用，重定向到OAuth2登录页面
        if settings.OAUTH2_ENABLED and settings.OAUTH2_AUTHORIZATION_URL:
            from fastapi.responses import RedirectResponse
            oauth2_login_url = f"{settings.API_V1_STR}/oauth2/login"
            # 保存原始请求URL，用于登录后跳转
            redirect_uri = urllib.parse.quote(str(request.url))
            return RedirectResponse(url=f"{oauth2_login_url}?redirect={redirect_uri}")
