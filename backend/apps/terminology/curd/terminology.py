...
def save_embeddings(session: Session, ids: List[int]):
    ...
    try:
        ...
        SQLBotLogUtil.exception("Failed to update terminology embeddings")
...
def get_terminology_template_with_data(session: SessionDep, question: str, oid: Optional[int] = 1,
                                       datasource: Optional[int] = None) -> tuple[str, List[Terminology]]:
    """
    获取术语模板和原始数据
    Returns:
        tuple: (template_string, terminology_list)
    """
    if not oid:
        oid = 1
    _results = select_terminology_by_word(session, question, oid, datasource)
