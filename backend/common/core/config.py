...
class Settings(BaseSettings):
    ...
    EMBEDDING_TERMINOLOGY_ENABLED: bool = Field(default=True, description="是否启用术语 Embedding 检索，关闭后仅使用 ILIKE 模糊匹配")
    ...
    ANALYSIS_DATA_ROW_LIMIT: int = Field(default=100, description="数据分析时传入 LLM 的最大数据行数，防止明细表上下文爆炸")
    ...
    GUESS_RECOMMEND_ENABLED: bool = Field(default=False, description="是否启用猜你想问/推荐问题模块总开关")
