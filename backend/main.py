...
from common.utils.utils import SQLBotLogUtil, _async_log_util
...
async def lifespan(app: FastAPI):
    ...
    yield
    # 刷新并关闭异步日志处理器
    _async_log_util.flush()
    _async_log_util.shutdown()
...
# CORS中间件必须在最外层，确保OPTIONS请求能正确处理
cors_origins = settings.all_cors_origins
if not cors_origins:
    # 如果没有配置，至少允许前端地址
    cors_origins = [settings.FRONTEND_HOST] if settings.FRONTEND_HOST else ["*"]
...
SQLBotLogUtil.info(f"CORS enabled origins: {cors_origins}")
