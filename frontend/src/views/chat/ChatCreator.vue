...
function showDs() {
  listDs().then((res) => {
    ...
    if (res && res.length > 0) {
      const firstDs = res[0]
      // 自动选择第一个数据源并创建对话
      autoCreateChatWithFirstDs(firstDs.id)
    } else {
      // 没有数据源，显示选择对话框
...
// 自动使用第一个数据源创建对话
function autoCreateChatWithFirstDs(datasourceId: number) {
  statusLoading.value = true
  // 先检查数据源是否可用
  datasourceApi
    .check_by_id(datasourceId)
    .then((res: any) => {
      if (res) {
        // 数据源可用，直接创建对话
        createChat(datasourceId)
      } else {
        // 数据源不可用，显示选择对话框让用户选择
        datasourceConfigVisible.value = true
        innerDs.value = datasourceId // 预选第一个数据源
      }
    })
    .catch((error) => {
      console.error('Check datasource failed:', error)
      // 检查失败，显示选择对话框
      datasourceConfigVisible.value = true
      innerDs.value = datasourceId // 预选第一个数据源
    })
