      ...
      <el-footer v-if="computedMessages.length > 0 || !isCompletePage" class="chat-footer">
        ...
        >
          <div
            ...
            :key="index"
            style="padding: 5px; border-bottom: 1px solid #f8f9fa; cursor: pointer"
...
import RecommendQuestion from './RecommendQuestion.vue' //猜你想问的组件

import ChatListContainer from './ChatListContainer.vue' //左边历史记录组件

import ChatCreator from '@/views/chat/ChatCreator.vue' //创建弹窗
...
import { promptApi } from '@/api/prompt2'
...
const props = withDefaults(
  ...
  {
    // 为 startChatDsId 设置默认值（例如默认值为 0 或 null 等，根据业务需求调整）
