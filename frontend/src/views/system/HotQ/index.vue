...
import { hot_Question_Api } from '@/api/prompt2'
...
const exportBatchUser = () => {
  ...
  ElMessageBox.confirm(
    ...
    hot_Question_Api.deleteEmbedded(multipleSelectionAll.value.map((ele) => ele.id)).then(() => {
...
const deleteHandler = (row: any) => {
  ...
  hot_Question_Api.deleteEmbedded({id:row.id}).then(() => {
  ...
  //   hot_Question_Api.deleteEmbedded([row.id]).then(() => {
...
<template>
    ...
    >
      ...
      <div class="preview-or-schema">
        ...
        >
          ...
          <el-table-column prop="sortNum" label="序号" width="280">

          </el-table-column>

          <el-table-column prop="staffNo" label="匹配工号" width="280">

          </el-table-column>

          <el-table-column prop="prompt" label="热门问题" min-width="240">

            <template #default="scope">

              <div class="field-comment_d">

                <span :title="scope.row.title" class="notes-in_table">{{ scope.row.title }}</span>
  ...
  >
    ...
    <el-form-item prop="sortNum" label="序号">

        <el-input-number

          v-model="pageForm.sortNum"
    ...
    color: #aaaaaa;">输入数字越小显示越靠前</span>

        </el-form-item>

      <el-form-item prop="name" label="匹配工号">

        <el-input

        disabled

          v-model="pageForm.staffNo"

          placeholder=" 请输入你想匹配给谁的工号"
      ...
      <el-form-item prop="prompt" label="热门问题">
