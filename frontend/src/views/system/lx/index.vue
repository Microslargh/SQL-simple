...
const deleteHandler = (row: any) => {
  promptApi.deleteEmbedded({id:row.id}).then(() => {
    ...
    if (res) {
      ...
      if(!obj.id){
        const value={
          ...
          promptContent:obj.promptContent, //工号
        }
        promptApi
          .addEmbedded(value)
          .then(() => {
      ...
      <div class="preview-or-schema">
        ...
        >
          ...
          <el-table-column prop="prompt" :label="$t('prompt.prompt_word_content')" min-width="240">
            <template #default="scope">
              <div class="field-comment_d">
                <span :title="scope.row.promptContent" class="notes-in_table">{{ scope.row.promptContent }}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column
  ...
  >
    ...
    >
      ...
      <el-form-item prop="prompt" :label="t('prompt.prompt_word_content')">
        <el-input
          v-model="pageForm.promptContent"
          placeholder="输入联想提示词"
