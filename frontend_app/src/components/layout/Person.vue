...
const name = computed(() => {
  console.log('[ userStore.getName ] >', userStore.getName)
  console.log('[ userStore.jobnumber ] >', userStore.jobnumber)
  return userStore.getName
  })
const uerName = computed(() => userStore.uerName)
const account = computed(() => userStore.getAccount)
const jobnumber = computed(() => userStore.jobnumber)
  ...
  >
    <template #reference>
      <button class="person" :title="name" :class="collapse && 'collapse'">
        ...
        <span v-if="!collapse" class="name ellipsis">{{ uerName||name }}</span>
      </button></template
    >
    <div class="popover" v-if="false">
      <div class="popover-content">
        <div class="info">
          <el-icon class="avatar-custom" size="30">
            <Default_avatar_custom></Default_avatar_custom>
          </el-icon>
          <div :title="name" class="top ellipsis">{{ uerName||name }}</div>
          <div :title="account" class="bottom ellipsis">{{ jobnumber||account }}</div>
        </div>
        <div  v-if="false" class="popover-item"  >
          <el-icon style="color: #646a73" size="16" @click="toSystem">
        ...
        <el-popover :teleported="false" v-if="false" popper-class="system-language" placement="right">
          <template #reference>
        ...
        <div v-if="false" class="popover-item" @click="toAbout">
          <el-icon size="16">
        ...
        <div class="popover-item mr4" v-if="false" @click="logout">
          <el-icon size="16">
