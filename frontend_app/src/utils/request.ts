...
class HttpService {
  ...
  private setupInterceptors() {
    ...
    this.instance.interceptors.request.use(
      (config: InternalAxiosRequestConfig) => {
        // Add auth token
        config.headers['AuthToken'] = 'af59237535844ec49d50a227e0162c8d'
        const token = wsCache.get('user.token')
        console.log(wsCache.get('user.token'),'wsCache.get-X-SQLBOT-TOKEN')
  ...
  private handleError(error: AxiosError) {
    ...
    console.log(error,'handleErrorerror')
    if (error.response) {
...
export const requestF = new HttpService({
  baseURL: import.meta.env.MODE === 'development' ? '' : (import.meta.env.VITE_API_BASE_URLR || ''),
})
/* 
