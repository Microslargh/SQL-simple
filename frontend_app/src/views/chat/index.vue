<template>
  ...
  <el-container class="chat-container no-padding">
    ...
    <el-container :loading="loading">
      ...
      <icon_send_mP style="height:110px" />
        <p style="text-align: center;">{{formattedTime}}</p>
        <p style="    text-align: center;
    font-size: 22px;">松开发送</p>
      </div>
      <div v-if="showSuggestions && suggestions.length > 0" style="width: 99%;
            border-radius: 6px;
            max-width: 800px;
        ...
        :key="index"
        @click="handleSelect(item)" style="padding: 5px;border-bottom: 1px solid #f8f9fa;cursor: pointer;"> {{ item }}</div>
      </div>
        <div class="input-wrapper" @click="clickInput">
          <div v-if="isCompletePage" class="datasource" style="display: flex;
            ...
            <div>
              ...
              <div  
               ...
               style="    display: flex;
                ...
                transition: opacity 0.1s ease;" size="30":style="{ opacity: currentOpacity }">
                <icon_send_mP /> 
                </el-icon>
                <p style="color: #a7afb5;font-size:10px">摁住录音</p>
            </div>
...
import { computed,watch , nextTick, onMounted, ref,onUnmounted  } from 'vue'
...
import icon_send_mP from '@/assets/svg/mP.svg'
...
// 动画方向：true为淡出，false为淡入
const isFadingOut = ref(true);
// 动画帧ID
...
const showPermissionPrompt = ref(true); // 是否显示权限请求提示
...
const handleInput = () => {
  ...
  // 防抖：300ms后请求后端（避免输入过程中频繁请求）
  debounceTimer = setTimeout(async () => {
    // debugger
    if (inputMessage.value.length < 2) { // 输入长度至少2个字符才请求
      suggestions.value = [];
      return;
    }
    ...
    try {
      // 请求后端接口获取联想数据
      promptApi
          .getMatch(inputMessage.value)
          .then((res) => {
            console.log('[ res ] >', res)
            suggestions.value = res.matchList || []; // 假设后端返回格式为 { data: string[] }
...
const isVisible = ref(false); // 控制计时显示是否可见
const isCompleted = ref(false); // 标记是否达到1分钟上限
...
const startTimer = () => {
  if (seconds.value >= 60) return; // 防止超过1分钟继续计时
...
const stopTimer = () => {
  ...
  isVisible.value = false; // 停止后隐藏计时显示
};
  ...
  try {
    ...
    inputMessage.value=''
      const formData = new FormData()
        ...
        // 将Blob转换为File对象，便于设置文件名 [citation:7]
        const audioFile = new File([blob], `recording-${new Date().getTime()}.wav`, {
          type: 'audio/wav'
        })
        ...
        // 发送上传请求 [citation:2]
        let url=import.meta.env.MODE==='development'? '/fin/cud/other/audioToText':import.meta.env.VITE_API_BASE_URLR+'/fin/cud/other/audioToText'
        await axios.post( url, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
            'AuthToken': 'af59237535844ec49d50a227e0162c8d'
          },
          ...
          inputMessage.value = data.asrcText;
        })
        // this.uploadProgress = 100

  } catch (error) {
    console.error('WAV 音频上传失败', error);
    // 失败后的处理（如重试提示）
  }
...
const checkMicrophonePermission = async () => {
  ...
  try {
    ...
    if (permissionStatus.state === 'granted') {
      ...
      errorMessage.value = '麦克风权限已被拒绝，请在浏览器设置中启用';
    ...
    errorMessage.value = '检查权限时发生错误，请刷新页面重试';
...
const computedMessages = computed<Array<ChatMessage>>(() => {
  ...
  console.log('[ currentChat.value ] >', currentChat.value.records)
  ...
  console.log('[ computedMessages ] >', messages)
...
const goEmpty = (func?: (...p: any[]) => void, ...param: any[]) => {
  ...
  console.log('[ currentChat.value ] >', currentChat.value.records)
  console.log('[ chatListSideBarShow ] >', computedMessages.value,loading.value,  )
}
